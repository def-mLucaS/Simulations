...
class VesselGravity(QtWidgets.QDialog):

    def __init__(self, ui):
        super(VesselGravity, self).__init__()
        ...
        # Eventos janela de configuração da embarcação
    ...
    def vessels_options(self):
        """Define as opções possíveis para a fundação gravidade"""
    ...
    def filter(self):
        ...
        try:
            ...
            if vessel_type == "WTIV":
                ...
                list_models = self.table_vessels["rebocador"]["nome_embarcacao"].values.tolist(
                )
                list_models.sort()
                self.add_list_vessels(
                    self.table_vessels["rebocador"], list_models, vessel_type)
    ...
    def select_vessel(self):
        """Adiciona embarcações na soluçlão de embarcação"""
            ...
            if not selected_vessel:
                ...
                if vessel is not None and vessel_type is not None:
                    ...
                    if vessel_type == "WTIV":
                        ...
                        df_vessel = self.table_vessels["rebocador"]
